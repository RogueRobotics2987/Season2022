...
extern "C" cap_cv* get_capture_webcam(int index)
{
    cv::VideoCapture* cap = NULL;
    try {
	cerr << "Trying to open webcam and set resolution - cknutson\n";
        cap = new cv::VideoCapture(index);
       cap->set(cv::CAP_PROP_FRAME_WIDTH, 640); // cknutson commented this back in to reduce video resolution
       cap->set(cv::CAP_PROP_FRAME_HEIGHT, 480);
	cerr << "Done setting resolution - cknutson\n";
    }
    catch (...) {
